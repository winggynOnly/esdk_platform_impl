...
public class DeviceConnManager implements IDeviceConnManager
{
    ...
    public void removeFromKeepAliveSchedule(IDeviceConnection conn)
    {
        ...
        {
            // 将其从保活列表中移除
            scheduler.pauseTrigger("cron_" + conn.getAdditionalData("connId") + "_"
                + conn.getAdditionalData("deviceId"),
                "eSDK_Jobs");
            scheduler.unscheduleJob("cron_" + conn.getAdditionalData("connId") + "_"
                + conn.getAdditionalData("deviceId"),
                "eSDK_Jobs");
            scheduler.deleteJob(conn.getAdditionalData("connId") + "_" + conn.getAdditionalData("deviceId")
