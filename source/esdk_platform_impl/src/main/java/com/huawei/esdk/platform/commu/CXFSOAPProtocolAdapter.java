...
public class CXFSOAPProtocolAdapter implements ICXFSOAPProtocolAdapter
{
        ...
        else
        {
            ...
            if(url.startsWith(ESDKConstant.PROTOCOL_ADAPTER_TYPE_HTTPS))
            {
                ...
                if (null == tlsParams)
                {
                    ...
                    tlsParams.setSecureSocketProtocol(ESDKConstant.SSL_SECURE_SOCKET_PROTOCOL_TLS);
                }
                ...
                SSLContext sslContext = SSLContext.getInstance(ESDKConstant.SSL_SECURE_SOCKET_PROTOCOL_TLS);
    ...
    public Object syncSendMessageWithCxf(Class<?> itfClass, String serviceApi, Object... reqMessage) throws ProtocolAdapterException
    {
        try
        {
            ...
            int len = reqMessage.length;
            Class<?>[] parameterTypes = new Class<?>[len];
            for (int i = 0; i < len; i++)
            {
                parameterTypes[i] = reqMessage[i].getClass();
            }
            
            Method m = proxy.getClass().getMethod(serviceApi, parameterTypes);
            Object res = m.invoke(proxy, reqMessage);
